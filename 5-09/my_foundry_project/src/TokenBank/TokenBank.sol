...
contract TokenBank is ReentrancyGuard {
    ...
    function depositEth() public payable nonReentrant {
        require(msg.value > 0.001 ether, "TokenBank: amount must be greater than 0.001 ether");
    ...
    function depositToken(address token, uint256 amount) public nonReentrant {
        ...
        require(balance >= amount, "TokenBank: insufficient token balance");
        
        // 添加授权检查
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= amount, "TokenBank: insufficient allowance");
     ...
     * @return 总 Token 存款量
     */
    function getTotalTokenDeposits(address token) public view returns (uint256) {
