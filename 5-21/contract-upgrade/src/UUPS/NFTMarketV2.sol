    ...
    EIP712Upgradeable 
{
    ...
    function list(address nftContract, uint256 tokenId, uint96 price) external nonReentrant {
        ...
        // 支持两种授权方式上架（isApprovedForAll｜Approve）
    ...
    function listWithSignature(
        ...
        // 验证NFT授权
        bool isApprovedForAll = IERC721(nftContract).isApprovedForAll(msg.sender, address(this));
        require(isApprovedForAll, "NFTMarket: not approved for all");
        ...
        require(signer != address(0), "NFTMarket: invalid signature");
        require(signer == msg.sender, "NFTMarket: unauthorized");
        ...
        emit NFTListedWithSignature(nftContract, tokenId, msg.sender, price, signer);
    }
